...
| `knip.json` | Reglas para detección de código muerto, puntos de entrada y rutas ignoradas |
...
```text
├── .github/             # Flujos CI/CD (CI, Lanzamientos) y configuración Dependabot
├── .storybook/          # Configuración de Storybook, archivo principal y vistas previas
├── .vscode/             # Configuración del editor, extensiones recomendadas y anidamiento
├── public/              # Archivos estáticos (tipografías, iconos, imágenes públicas)
├── src/
│   ├── app/             # Páginas, endpoints de API, layouts y Server Actions de App Router
│   ├── components/      # Componentes UI modulares e historias de Storybook (*.stories.tsx)
│   ├── database/        # Esquemas de Drizzle, migraciones, seeders y conexión
│   ├── lib/             # Utilidades, logger estructurado, validación env y config i18n
│   ├── locales/         # Diccionarios de traducción en JSON
│   ├── store/           # Slices de estado global del cliente (Zustand)
...
├── tests/               # Suites E2E de Playwright y mocks de API con MSW
├── architecture.md      # Documento detallado de arquitectura del sistema
├── drizzle.config.ts    # Configuración del ORM de base de datos
├── knip.json            # Configuración del analizador de código muerto
